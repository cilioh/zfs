...
#endif
		if (highbit64(stage)-1 == 6){
			chksm_zio_taskq_dispatch(zio, ZIO_TAKSQ_ISSUE, B_FALSE);
			stage <<= 1;
		}
//
...
hrtime_t iia_local[2];
iia_local[0] = gethrtime();
...
iia_local[1] = gethrtime();
calclock(iia_local, &iia_t, &iia_c);
...
hrtime_t iibb_local[2];
iibb_local[0] = gethrtime();	
	if (bp != NULL && bp != &zio->io_bp_copy)
		zio->io_bp_copy = *bp;
iibb_local[1] = gethrtime();
calclock(iibb_local, &iibb_t, &iibb_c);
	

hrtime_t iibc_local[2];
iibc_local[0] = gethrtime();	
	if (zio->io_error != 0) {
		zio->io_pipeline = ZIO_INTERLOCK_PIPELINE;
...
iibc_local[1] = gethrtime();
calclock(iibc_local, &iibc_t, &iibc_c);
	
iib_local[1] = gethrtime();
calclock(iib_local, &iib_t, &iib_c);
	
hrtime_t iic_local[2];
iic_local[0] = gethrtime();
	
hrtime_t enter_local[2];
enter_local[0]  = gethrtime();
	mutex_enter(&zio->io_lock);
enter_local[1] = gethrtime();
calclock(enter_local, &enter_t, &enter_c);

	zio->io_state[ZIO_WAIT_READY] = 1;
	pio = zio_walk_parents(zio, &zl);

hrtime_t exit_local[2];
exit_local[0]  = gethrtime();	
	mutex_exit(&zio->io_lock);
exit_local[1] = gethrtime();
calclock(exit_local, &exit_t, &exit_c);


iic_local[1] = gethrtime();
calclock(iic_local, &iic_t, &iic_c);
	
hrtime_t iid_local[2];
iid_local[0] = gethrtime();
...
iid_local[1] = gethrtime();
calclock(iid_local, &iid_t, &iid_c);
